...
- `src/models.py`: schema-v12 `Body`, `FlybyData`, `SystemGroup`, `SystemReferenceFrame`, and `SolarSystem` dataclasses with validation, hierarchy, migration, canonical-state provenance, canvas/path settings, and JSON conversion.
...
Schema v12 adds persisted orbit-guide visibility, recorded-trail visibility, path style, and Fixed Scale view-mode settings. It retains v11 flyby inputs, v10 trail-frame migration, and v9 body-provenance/reference-frame migration. Older documents receive the new canvas defaults during load. A body can have only one direct group owner.
