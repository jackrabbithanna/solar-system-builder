...
The detailed coordinate model, transformation pipeline, frame schema, analysis kinematics, and export contracts are documented in [Reference Frames and Coordinate Transformations](./REFERENCE_FRAMES.md).
...
- `src/models.py`: schema-v14 `Body`, `FlybyData`, `SystemGroup`, structured `SystemReferenceFrame`/`ReferenceOrigin`, and `SolarSystem` dataclasses with validation, hierarchy, migration, canonical-state provenance, physics/integrator settings, canvas/path settings, and JSON conversion.
- `src/documents.py`: pure validated JSON document parsing, canonical serialization, and collision-safe imported names.
- `src/standard_frames.py`: Astropy/PyERFA-backed epoch, time-scale, precession/nutation, ecliptic, equatorial, ICRF, and Galactic axes definitions with offline IERS data.
- `src/reference_frames.py`: pure rigid transforms plus full-state epoch propagation, verified standard axes/origins, barycenter origins, matrix validation, and osculating metadata rebuilding.
- `src/analysis_frames.py`: session-only translating and rotating frame kinematics, target-pair co-rotation, coordinate transforms, and explicit apparent-acceleration diagnostics.
- `src/frame_exports.py`: importable relative JSON snapshots and full relative-state/non-inertial CSV diagnostics.
...
5. `playback.SimulationSession` builds copied full/active/overview/context `SimulationState` data and immutable `SimulationJob` records carrying the selected gravity model, integrator, and optional analysis-frame specification. At star-scale detail, Auto and scoped body policies replace each planet and its moons with a mass-weighted proxy; completed proxy deltas translate every member so local moon state remains intact. Hybrid focus combines focused bodies and coarse context into one temporary coupled state, then splits completed results so context gravity affects the focus without merging context proxies into the body model. Playback advances worker copies through state-sampling helpers and derives analysis-frame trail arrays in that worker, leaving the GTK thread to apply completed arrays. Focused-parent trails translate each sample against the focused body or group barycenter before storage.
...
12. `SystemLibraryController` coordinates explicit preset duplication, saved-system persistence, guarded selection, and deletion. Portable imports are duplicated with regenerated linked IDs before entering the library; relative exports materialize a cloned current state and can re-center it without mutating the library.
13. Verified reference-frame edits stop playback, run full N-body propagation and any serialized JPL origin fetch in a worker, then atomically rotate and translate every state and recompute osculating metadata. Custom matrices remain labeled custom rather than being trusted as standard axes. Dynamic translating/rotating analysis frames affect the main canvas, trails, diagnostics, and CSV output only; canonical physics and the navigation inset remain inertial.

`Body.parent_id` records local orbital/display parentage. Stars are roots; bound planets, dwarf planets, comets, and asteroids orbit stars; moons orbit planets or dwarf planets. Non-moon bodies may also be unparented Cartesian or flyby bodies. `Body.state_origin` records whether the current canonical vector came from Cartesian entry, orbital generation, Horizons, or the flyby builder. Optional `Body.orbit`, `Body.flyby`, and `Body.data_source` preserve provenance, while `position_m` and `velocity_mps` remain canonical. `SolarSystem.reference_frame` stores a verified axes identifier and structured origin together with epoch and time scale. `SystemGroup` records larger semantic hierarchy, but physics remains a flat N-body simulation over the active body set.

Schema v14 replaces free-form standard-frame labels with registered axes and structured origins while retaining legacy migration. It retains v13 gravity/integrator settings, v12 canvas/path settings, v11 flyby inputs, v10 trail-frame migration, and v9 body provenance. A body can have only one direct group owner.
...
- `test_standard_frames.py`: time scales, standard axes, precession/nutation, and epoch-aware state transforms.
- `test_analysis_frames.py`: translating/prescribed/co-rotating frames and explicit non-inertial terms.
- `test_frame_exports.py`: importable relative snapshots and CSV diagnostic columns.
