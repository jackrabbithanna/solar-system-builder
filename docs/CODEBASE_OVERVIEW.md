...
- `src/models.py`: schema-v10 `Body`, `SystemGroup`, `SystemReferenceFrame`, and `SolarSystem` dataclasses with validation, hierarchy, migration, canonical-state provenance, trail-frame settings, and JSON conversion.
...
Schema v10 adds the persisted trail-frame preference and defaults older systems to focused-parent trails. It also retains the v9 migration of older body provenance and reference-frame metadata. The bundled Solar System and Dwarf Planets presets migrate to their known JPL solar-system-barycentric frame; other legacy systems migrate to an app-local frame. A body can have only one direct group owner.
