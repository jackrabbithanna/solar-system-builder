...
The canvas playback badge is also main-thread state. It shows continuous playback while jobs are being queued and distinguishes a paused playback loop from an already-running worker step that still needs to finish. Worker callbacks never update the badge directly; completion returns through `GLib.idle_add` before the icon, label, spinner, or visibility changes.
